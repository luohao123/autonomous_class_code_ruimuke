...
```
git clone http://gitlab.com/StrangeAI/centernet_pro_max
cd centernet_pro_max
...
- *2020.04.14*: Now tensorrt engine can be generated:

  ```
  check on engine: ctdet_r50_coco.trt
  => checking engine....
  => engine maxBatchSize: 32
  => engine NbBindings: 4
      => BindingName at: 0=input.1 Dims=4 shape: 1,3,448,672,
      => BindingName at: 1=533 Dims=4 shape: 1,80,112,168,
      => BindingName at: 2=536 Dims=4 shape: 1,2,112,168,
      => BindingName at: 3=539 Dims=4 shape: 1,2,112,168,
  done.
  engine shutdown.
  ```

  Need check it consistency with TensorRT implementation. Also process procedure needs to alt to 512x512 input;

- *2020.04.10*: DCNv2 supports onnx export, will make it forward soon; 

- *2020.04.09*: Removed dcn default convert to torchvision one, also DCNv2 supported;
...
- [ ] Try adding https://github.com/hkzhang95/DynamicRCNN to this repo.
