...
from .backbone.shufflenetv2_backbone import ShuffleNetV2Backbone
...
class CenterNet(nn.Module):
            ...
            if self.cfg.MODEL.ONNX:
                assert isinstance(batched_inputs,
                                  dict), 'for ONNX mode, inputs should' \
                                                 ' dict contains aligned_img and img_info'
                aligned_img = batched_inputs['aligned_img']
                return self.inference_onnx(aligned_img)
            else:
    ...
    def inference(self, images):
        """
        image(tensor): ImageList in .structures
        """
        ...
        # logging.info('backbone in image shape: {}'.format(aligned_img.shape))
    ...
    def preprocess_onnx(self, images, images_info, target_shape=None):
        h, w = images_info[0]
        c = 3
        ...
        if target_shape != None:
            ...
            images = [torch.tensor(img).permute((2, 0, 1)).to(device) for img in images]
            images = [self.normalizer(img / 255) for img in images]
            images = torch.tensor(images[0]).unsqueeze(0).to(device)
    ...
    @staticmethod
    def decode_prediction_onnx(pred_dict):
        """
        Args:
            pred_dict(dict): a dict contains all information of prediction
            img_info(dict): a dict contains needed information of origin image
        ...
        reg = pred_dict["reg"]
        wh = pred_dict["wh"]
        return CenterNetDecoder.decode(fmap, wh, reg)
